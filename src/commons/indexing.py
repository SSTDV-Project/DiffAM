...
def batch_scatter_add_(v, i, s, dim: int = 1):
    """
    v: tensor (*B, m, *n)
    i: int tensor (*B, *k)
    s: tensor (*B, *k, *n)
    ...
    vv.scatter_add_(0, iii.view(-1), ss)
...
def batch_index_reduce_(v, i, s, dim: int = 1, reduce = 'amin', include_self=True):
    ...
    vv.index_reduce_(0, iii.view(-1), ss, reduce, include_self=include_self)
...
def multibatch_mm(a, b):
    ...
    *B, n, m = a.shape
    *Bb, mb, p = b.shape
    assert (not B or not Bb or B == Bb) and m == mb
    rB = B or Bb

    aa = a.view(-1, n, m)
    bb = b.view(-1, m, p)
    rr = torch.matmul(aa, bb)
    return rr.view(*rB, n, p)
